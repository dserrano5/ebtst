...
#TODO: hit_no2. Need a hits CSV with moderated hits
...
for (0..6) { ok $res->{'hit_list'}[$_]{'note_no'} - $hit_note_nos[$_] < 0.001, sprintf 'Correct hit %d note number', $_ + 1; }

my @hit_notes_elapsed = qw/4 6 8 9 10 10 11/;
for (0..6) { ok $res->{'hit_list'}[$_]{'notes'} - $hit_notes_elapsed[$_] < 0.001, sprintf 'Correct hit %d notes elapsed', $_ + 1; }

my @hit_old_ratios = (undef, qw/5 4 3 2.25 2 1.6667/);
is $res->{'hit_list'}[0]{'old_hit_ratio'}, undef, 'Correct hit 1 old ratio';
for (1..6) { ok $res->{'hit_list'}[$_]{'old_hit_ratio'} - $hit_old_ratios[$_] < 0.001, sprintf 'Correct hit %d old ratio', $_ + 1; }

my @hit_new_ratios = qw/4 3 2.6667 2.25 2 1.6667 1.5714/;
for (0..6) { ok $res->{'hit_list'}[$_]{'new_hit_ratio'} - $hit_new_ratios[$_] < 0.001, sprintf 'Correct hit %d new ratio', $_ + 1; }
